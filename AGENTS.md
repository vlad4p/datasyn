...
**Warehouse file lock (`warehouse.duckdb`):** DuckDB allows **only one process at a time** to open the native database file for read-write ([concurrency](https://duckdb.org/docs/current/connect/concurrency.html)). On the fleet, **`duckdb-mcp`** on VM1 holds the **single RW session** to `warehouse.duckdb` and serves it over the **Quack** remote protocol (`:9494`). Dagster (VM3) and other writers attach via **`QUACK_URI`** / **`QuackDuckDBResource`** — they do **not** mount `duckdb_data`. **`duckdb_get_schema`** / **`duckdb_execute_query`** (MCP) use the same `duckdb-mcp` process. The optional **DuckDB Local UI** (`duckdb-ui`, compose profile `ui`) also opens the file and conflicts with **`duckdb-mcp`** — stop one before using the other. Send **one SQL statement per `duckdb_execute_query`** call (do not chain two `CREATE TABLE` statements in one string).

**Dagster bronze ingest from MinIO:** Dagster assets read landing objects with **`read_csv_auto('s3://…')`** via **httpfs** (S3 secret → MinIO on VM2) on the client session, then materialize into **`bronze.*`** on the remote warehouse through Quack (`USE warehouse`). No boto3 temp-file download for CSV bronze loads.
