"""Clarín: scrape → MinIO ``landing/clarin/`` (markdown) → ``bronze.clarin_noticias``.

Particiones diarias: el hub no trae la fecha en la URL; se conservan notas cuyo
``article:published_time`` cae en el día UTC de la partición (misma convención
que el ``partition_key`` YYYY-MM-DD al materializar).
"""
...
from datasyn.assets.bronze.clarin.lib import (
    SECTIONS,
    build_markdown_file,
...
TABLE_NAME = "clarin_noticias"

CLARIN_DAILY = DailyPartitionsDefinition(
...
@asset(
    ...
    compute_kind="python",
    description=(
        "Scrape Clarín (política, economía, rural) para la fecha de partición; "
        "sube markdown a ``landing/clarin/<tema>/YYYY/MM/DD/*.md`` en MinIO."
    ),
)
def clarin_landing_markdown(context: AssetExecutionContext) -> MaterializeResult:
    day = _partition_date(context)
    context.log.info("clarin scrape partition=%s", context.partition_key)
...
@asset(
    deps=[clarin_landing_markdown],
    partitions_def=CLARIN_DAILY,
    group_name="bronze",
    compute_kind="duckdb",
    description=(
        "Lee ``landing/clarin/*/YYYY/MM/DD/*.md`` en MinIO para la partición, "
        f"borra filas de ese día en ``{BRONZE_SCHEMA}.{TABLE_NAME}`` e inserta "
        "(Iceberg REST si está configurado)."
    ),
)
def clarin_noticias_bronze(context: AssetExecutionContext, database: DuckDBResource) -> MaterializeResult:
    day = _partition_date(context)
    y, m, dd = day.year, day.month, day.day
    prefix = "landing/clarin/"
