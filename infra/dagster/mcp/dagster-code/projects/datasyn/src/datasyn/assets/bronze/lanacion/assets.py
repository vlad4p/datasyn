"""La Nación: scrape → MinIO landing (markdown) → ``bronze.lanacion_noticias``.

Particiones diarias: se filtran enlaces cuyo path termina en ``-nidDDMMYYYY`` coincidente con la partición.
Fuentes: política, economía, editoriales, Buenos Aires; ``judiciales`` usa el hub ``/seguridad/`` (listado de crimen y tribunales).
"""
...
TABLE_NAME = "lanacion_noticias"

LAN_DAILY = DailyPartitionsDefinition(
...
@asset(
    ...
    compute_kind="python",
    description=(
        "Scrape La Nación (política, economía, editoriales, Buenos Aires, judiciales vía seguridad) "
        "para la fecha de partición; sube markdown a ``landing/lanacion/<tema>/YYYY/MM/DD/*.md`` en MinIO."
    ),
)
def lanacion_landing_markdown(context: AssetExecutionContext) -> MaterializeResult:
    day = _partition_date(context)
    context.log.info("lanacion scrape partition=%s", context.partition_key)
...
@asset(
    deps=[lanacion_landing_markdown],
    partitions_def=LAN_DAILY,
    group_name="bronze",
    compute_kind="duckdb",
    description=(
        "Lee objetos ``landing/lanacion/*/YYYY/MM/DD/*.md`` de MinIO para la partición, "
        f"reemplaza filas de ese día en ``{BRONZE_SCHEMA}.{TABLE_NAME}`` e inserta (Iceberg REST si está configurado)."
    ),
)
def lanacion_noticias_bronze(context: AssetExecutionContext, database: DuckDBResource) -> MaterializeResult:
    day = _partition_date(context)
    y, m, dd = day.year, day.month, day.day
    prefix = "landing/lanacion/"
