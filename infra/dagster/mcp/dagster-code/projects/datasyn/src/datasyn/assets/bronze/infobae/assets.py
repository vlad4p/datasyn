"""Infobae: scrape → MinIO landing (markdown) → ``bronze.infobae_noticias`` (Iceberg si aplica).

Particiones diarias: cada partición corresponde a un día calendario; se enlazan notas
cuya URL contiene ``/tema/YYYY/MM/DD/``. Job ``infobae_backfill_job`` materializa
``infobae_landing_markdown`` y luego ``infobae_noticias_bronze`` para el rango elegido
en el Launchpad (selección de particiones).
...
from datasyn.assets.bronze.infobae.lib import (
    SECTIONS,
    build_markdown_file,
...
TABLE_NAME = "infobae_noticias"

INF_DAILY = DailyPartitionsDefinition(
...
@asset(
    ...
    compute_kind="python",
    description=(
        "Scrape Infobae (política, judiciales, economía) para la fecha de partición, "
        "sube markdown a ``landing/infobae/<tema>/YYYY/MM/DD/*.md`` en MinIO."
    ),
)
def infobae_landing_markdown(context: AssetExecutionContext) -> MaterializeResult:
    day = _partition_date(context)
    context.log.info("infobae scrape partition=%s", context.partition_key)
...
@asset(
    deps=[infobae_landing_markdown],
    partitions_def=INF_DAILY,
    group_name="bronze",
    compute_kind="duckdb",
    description=(
        "Lee objetos ``landing/infobae/*/YYYY/MM/DD/*.md`` de MinIO para la partición, "
        f"reemplaza filas de ese día en ``{BRONZE_SCHEMA}.{TABLE_NAME}`` e inserta (Iceberg REST si está configurado)."
    ),
)
def infobae_noticias_bronze(context: AssetExecutionContext, database: DuckDBResource) -> MaterializeResult:
    day = _partition_date(context)
    y, m, dd = day.year, day.month, day.day
    prefix = f"landing/infobae/"
