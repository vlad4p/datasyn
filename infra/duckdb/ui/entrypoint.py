    ...
    *,
    ...
    while time.monotonic() < deadline:
        if worker is not None and worker.poll() is not None:
            log.error(
                "worker exited before DuckDB UI listened on localhost:%s (exit code=%s). "
                "If you saw a lock error: only one process may open %s read-write — stop "
                "duckdb-mcp, Dagster user code, or other DuckDB clients first, then restart duckdb-ui. "
                "https://duckdb.org/docs/stable/connect/concurrency.html",
                port,
                worker.returncode,
                DB_PATH,
            )
            raise RuntimeError("duckdb-ui worker died")
        attempt += 1
        if attempt % 25 == 1:
...
def _run_worker() -> None:
    ...
    try:
        ...
        if "could not set lock" in low or "conflicting lock" in low:
            log.error(
                "Cannot open warehouse — file is locked by another process (often **duckdb-mcp** "
                "on the same volume, Dagster, or a second DuckDB UI). Stop those services, then "
                "restart this container. Database path: %s — "
                "https://duckdb.org/docs/stable/connect/concurrency.html",
                DB_PATH,
            )
