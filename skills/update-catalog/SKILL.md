...
description: >-
  ...
  an existing dataset. Uses warehouse MCP to introspect schema, catalog MCP
  (catalog_execute_query) to upsert entity_json and refresh tag_usage /
...
The **brain** (this agent) is the only place that ties **DuckDB** and the **metadata catalog** together. **Never** connect the catalog database to the warehouse: there is no direct catalog→DuckDB link. You **read schema from DuckDB** via **`duckdb_*`** tools and **write metadata** as SQL through **`catalog_execute_query`**.
...
The catalog MCP exposes only **`catalog_get_schema`** and **`catalog_execute_query`** — there is **no** `catalog_register_or_update_dataset` / `catalog_set_lineage` / `catalog_get_dataset` tool. Every catalog read/write below is a SQL statement built by the brain and run through **`catalog_execute_query`** (one statement per call). See **`./skills/catalog-sql/SKILL.md`** for the base SQL shapes.

| Step | Server | Tool | Purpose |
|------|--------|------|---------|
| List objects | `duckdb` | **`duckdb_warehouse_list_tables`** | Find `table_schema`, `table_name`, `table_type`. |
| Column types | `duckdb` | **`duckdb_warehouse_query`** | `PRAGMA table_info('schema.table')` and/or `information_schema.columns`. |
| Optional counts | `duckdb` | **`duckdb_warehouse_query`** | `SELECT COUNT(*)`, `NULL` rates — only if useful for the description; keep **`LIMIT`** in mind. |
| Catalog layout | `catalog` | **`catalog_get_schema`** | Confirm table / column names on first run of a session. |
| Check existing | `catalog` | **`catalog_execute_query`** | `SELECT id, entity_json FROM dataset_entity WHERE fully_qualified_name = '<fqn>'` (or the FTS query from `catalog-sql`). |
| Upsert entity | `catalog` | **`catalog_execute_query`** | `INSERT INTO dataset_entity (fully_qualified_name, entity_json) VALUES ('<fqn>', '<json>'::jsonb) ON CONFLICT (fully_qualified_name) DO UPDATE SET entity_json = EXCLUDED.entity_json RETURNING id`. |
| Refresh tags | `catalog` | **`catalog_execute_query`** | Two statements (two calls): `DELETE FROM tag_usage WHERE target_id = <id>;` then one `INSERT INTO tag_usage …` per tag. |
| Refresh lineage | `catalog` | **`catalog_execute_query`** | Delete upstream/downstream edges for the dataset id, then `INSERT INTO entity_relationship …` per resolved FQN (never self-edges). |

Do **not** use PostgreSQL, `psql`, or a direct connection string to the **catalog** from here except through **`catalog_execute_query`** (that is the supported path).
...
Call **`duckdb_warehouse_query`** with read-only SQL, for example:
...
1. **Resolve target** — Table/view name or FQN. If unknown, **`duckdb_warehouse_list_tables`**, then match with the FTS query from **`./skills/catalog-sql/SKILL.md`** (`search_tsv @@ plainto_tsquery(...)`).
...
3. **Introspect** — `PRAGMA table_info(...)` (and optional stats) via **`duckdb_warehouse_query`**.
...
7. **Refresh `tag_usage`** — one `DELETE` call, then one `INSERT` per tag (each is its own `catalog_execute_query` call; the guard rejects multi-statement SQL).
...
- **Ingesting** or **`duckdb_data_local_ls`** just to "prepare" a catalog update — **not required** for registration if the table already exists in DuckDB; only use file paths the user or lineage gives you.
...
- **Claiming** that `catalog_register_or_update_dataset`, `catalog_set_lineage`, `catalog_list_datasets`, `catalog_search_datasets`, or `catalog_get_dataset` exist — they don't; the catalog MCP only exposes `catalog_get_schema` and `catalog_execute_query`.
- **Empty `columns` array** (`"columns": []`) when the warehouse table already exists — always run **`PRAGMA table_info('schema.table')`** or **`information_schema.columns`** via **`duckdb_warehouse_query`** first and populate **`name`**, **`dataType`**, **`description`** for every column (OpenData / data-dictionary expectation).
...
- Load data first (when needed): **`./skills/ingest-indec-mercadolaboral/SKILL.md`** for INDEC EPH under **`/data-local/indec/mercado_laboral/`**; for other file patterns use **`duckdb_warehouse_query`** with **`read_csv_auto`** / **`read_csv`** directly per **`AGENTS.md`** → **Ingest and delimited data**.
