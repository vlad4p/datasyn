...
| **Skills (`./skills/*/SKILL.md`)** | **Runtime:** Deep Agents loads the parent **`/skills/`** in `agent/graph.py` so every subfolder with `SKILL.md` is injected (INDEC ingest, scrape, update-catalog, etc.). **`./skills/langfuse/`** is maintainer/docs only (not injected into the agent). Not a separate microservice; no HTTP “skill server.” |
| **dagster-mcp** | Dagster scaffolding + Docker deploy helpers; also **optional PostgreSQL catalog** tools (`dagster_catalog_get_schema`, `dagster_catalog_execute_query`) when **`DATABASE_URL`** / **`CATALOG_DATABASE_URL`** is set on that service. |
