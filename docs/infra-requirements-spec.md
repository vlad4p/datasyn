...
| **Shared overlay network** | All VMs join the same logical Docker network **`infra-datasynk`** (create once: `make bootstrap` or `docker network create infra-datasynk`). In production, use a **VPN / private VPC** and point compose at an **external** network, or use Swarm/overlay plugins—see §5. |
...
### 5.1 Recommended VM list (one stack per VM)

```text
                    ┌─────────────────────────────────────────┐
                    │  Private network / VPN (all VMs)         │
                    │  Docker network: infra-datasynk          │
                    └─────────────────────────────────────────┘
        ┌──────────────┬──────────────┬──────────────┬──────────────┐
        ▼              ▼              ▼              ▼              ▼
   VM-minio      VM-duckdb      VM-dagster     VM-litellm     VM-langfuse
   :9000         :8040          :3001          :4000          :3000
        │              │              │              │              │
        └──────────────┴──────────────┴──────────────┴──────────────┘
                              MCP + SQL + jobs
        ┌──────────────┬──────────────┐
        ▼              ▼              ▼
   VM-brain       VM-ui          VM-gpu (optional)
   :8002          :8003          fine-tune workers
```
...
| 6 | Brain + UI | `make agent-up` or split services |

Full dev stack: `make stack-up` (`ENVIRONMENT=dev`) from repo root.
...
For local laptops, all stacks may run on **one host** (see `INSTALL.md`, `make stack-up`). This spec’s **per-VM** layout is the **target production** shape; sizing in §4 still applies by **summing** resources if you temporarily colocate.
...
| `make/deploy.mk` | `ENVIRONMENT=dev|prod`, shared volumes/network |
